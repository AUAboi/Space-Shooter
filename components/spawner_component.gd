...
func spawn(spawn_global_position: Vector2 = global_position, parent: Node = get_tree().current_scene) -> Node:
	assert(scene is PackedScene, "Erraor: The scene export was never set on this spawner component.")
	# Instance the scene
